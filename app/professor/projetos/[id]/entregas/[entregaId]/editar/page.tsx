...
import { buscarEntrega } from "@/lib/entregas";
import EntregaForm from "../../EntregaForm";

export default async function EditarEntregaPage({ params }: {
  ...
  try {
    ...
    entrega = await buscarEntrega(context, projeto.id, entregaId);
  } catch (error) {
    ...
    return (
      <main className="min-h-screen bg-[#F5F7FA] px-4 py-8 text-[#172033] sm:px-6 lg:px-10">
        <div role="alert" className="max-w-3xl rounded-2xl border border-[#172033]/10 bg-white p-6">
          <h1 className="text-xl font-bold">Não foi possível carregar a entrega</h1>
  ...
  return (
    <main className="min-h-screen bg-[#F5F7FA] px-4 py-6 text-[#172033] sm:px-6 sm:py-8 lg:px-10">
      <header className="border-b border-[#172033]/10 pb-6">
        <ProfessorGreeting />
        <h1 className="mt-1 font-[family-name:var(--font-manrope)] text-3xl font-bold">Editar entrega</h1>
        <p className="mt-2 break-words text-sm text-[#172033]/60">{projeto.titulo}</p>
      </header>
      <EntregaForm projetoId={projeto.id} entrega={entrega} />
    </main>
  );
}
